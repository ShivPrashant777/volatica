...
io.on('connect', function (socket) {
	...
	socket.on('joinRoom', function (data) {
		...
		socket.emit(
			'sendMsg',
			formatMessage('Admin', 'You are now connected to Volatica')
		);
		var greetMsg = data.username + ' has joined the chat';
		socket.broadcast
			.to(data.room)
			.emit('sendMsg', formatMessage('Admin', greetMsg));
	});
	...
	socket.on('getMsg', function (data) {
		if (data.msg != '') {
			...
			socket.broadcast
				.to(data.room)
				.emit('sendMsg', formatMessage(data.sender, data.msg)); // Send to all clients except user
		}
	});
	...
	socket.on('disconnect', function () {
		...
		io.in(NEWUSER.room).emit('getUsers', {
			room: NEWUSER.room,
